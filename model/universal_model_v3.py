...
Bridge health monitoring using ensemble ML methods:
- Tilt pairing: 2+2 pattern (X/Y axis mapping)
...
class UniversalModelV3:
    """
    Bridge health anomaly detection using ensemble ML methods.
    """

    # Sensor operating ranges
    SENSOR_RANGES = {
        'accelerometer': {'p2p': (-1.0, 1.0), 'rms': (-1.0, 1.0)},
        'displacement': {'value': (-500.0, 500.0)},
        'tilt': {'value': (-5.0, 5.0)},
        'temperature_probe': {'value': (-40.0, 125.0)}
    }

    # Tilt axis mapping (2+2 pattern: 2 X-axis, 2 Y-axis sensors)
    TILT_AXIS_MAPPING = {
    ...
    def __init__(self, structure_id=None, ensemble_vote_threshold=2):
        ...
        Args:
            ...
            ensemble_vote_threshold: Minimum votes to flag as anomaly
        """
        ...
        # Ensemble weights based on empirical performance
    ...
    def learn_seasonal_baselines(self, df):
        """Learn seasonal patterns from the data using MAD statistics."""
        ...
        if 'temperature_probe' in self.sensors_by_type:
            ...
            for month in range(1, 13):
                month_data = df_temp[df_temp['month'] == month]['value'].dropna()
                if len(month_data) > 0:
                    mean_val = month_data.mean()
                    std_val = month_data.std()
                    median_val = month_data.median()
                    mad = np.median(np.abs(month_data - median_val))
                    robust_std = mad * 1.4826 if mad > 0 else std_val
            ...
            baselines['temperature'] = {
                ...
                'overall_median': df_temp['value'].median()
            }
            print(f"  Temperature: learned 12-month baseline (mean: {baselines['temperature']['overall_mean']:.1f}°C, median: {baselines['temperature']['overall_median']:.1f}°C)")

        if 'displacement' in self.sensors_by_type:
            df_di = df[df['sensor_type'] == 'displacement']
            di_monthly = {}
            for month in range(1, 13):
                month_data = df_di[df_di['month'] == month]['value'].dropna()
                if len(month_data) > 0:
                    mean_val = month_data.mean()
                    std_val = month_data.std()
                    median_val = month_data.median()
            ...
            baselines['displacement'] = {
                ...
                'overall_median': df_di['value'].median()
            }
            print(f"  Displacement: learned 12-month baseline (mean: {baselines['displacement']['overall_mean']:.1f}mm, median: {baselines['displacement']['overall_median']:.1f}mm)")

        if 'tilt' in self.sensors_by_type:
            df_ti = df[df['sensor_type'] == 'tilt']
            ti_monthly = {}
            for month in range(1, 13):
                month_data = df_ti[df_ti['month'] == month]['value'].dropna()
                if len(month_data) > 0:
                    mean_val = month_data.mean()
                    std_val = month_data.std()
                    median_val = month_data.median()
            ...
            baselines['tilt'] = {
                ...
                'overall_median': df_ti['value'].median()
            }
            print(f"  Tilt: learned 12-month baseline (mean: {baselines['tilt']['overall_mean']:.2f}°, median: {baselines['tilt']['overall_median']:.2f}°)")
        ...
        if 'accelerometer' in self.sensors_by_type:
            ...
            rms_monthly = {}
            for month in range(1, 13):
                month_data_p2p = df_ac[df_ac['month'] == month]['p2p'].dropna()
                month_data_rms = df_ac[df_ac['month'] == month]['rms'].dropna()

                if len(month_data_p2p) > 0:
                    mean_val = month_data_p2p.mean()
                    std_val = month_data_p2p.std()
                    median_val = month_data_p2p.median()
                    mad = np.median(np.abs(month_data_p2p - median_val))
                    robust_std = mad * 1.4826 if mad > 0 else std_val
                ...
                if len(month_data_rms) > 0:
                    mean_val = month_data_rms.mean()
                    std_val = month_data_rms.std()
                    median_val = month_data_rms.median()
                    mad = np.median(np.abs(month_data_rms - median_val))
                    robust_std = mad * 1.4826 if mad > 0 else std_val
            ...
            baselines['accelerometer'] = {
                ...
                'p2p_median': df_ac['p2p'].median(),
                'rms_median': df_ac['rms'].median()
            }
            print(f"  Accelerometer: learned 12-month baseline with robust statistics")
    ...
    def get_seasonal_deviation(self, value, sensor_type, month, metric='value'):
        """Calculate deviation from seasonal baseline using robust statistics."""
        ...
        month_baseline = monthly_data[month]

        if 'median' in month_baseline and 'robust_std' in month_baseline:
            expected_median = month_baseline['median']
            expected_robust_std = month_baseline['robust_std']

            if expected_robust_std == 0 or np.isnan(expected_robust_std):
                return 0

            return (value - expected_median) / expected_robust_std
        else:
            expected_mean = month_baseline.get('mean', 0)
            expected_std = month_baseline.get('std', 1)
    ...
    def detect_di_pairing(self, df):
        ...
        for s1, s2 in combinations(di_sensors, 2):
            ...
            def calculate_mad_robust_std(values):
                """Calculate MAD-based robust standard deviation."""
                median_val = values.median()
                mad = np.median(np.abs(values - median_val))
                return mad * 1.4826 if mad > 0 else values.std()

            early_robust = calculate_mad_robust_std(early)
            middle_robust = calculate_mad_robust_std(middle)
            late_robust = calculate_mad_robust_std(late)

            early_median = early.median()
            late_median = late.median()
            total_drift = abs(late_median - early_median)

            within_period_robust_avg = (early_robust + middle_robust + late_robust) / 3
            ...
            criterion1 = within_period_robust_avg < 20
            criterion2 = total_drift < 50
            criterion3 = rolling_std_median < 5

            is_paired = criterion1 and criterion2 and criterion3

            if is_paired and within_period_robust_avg < best_within_std:
                best_within_std = within_period_robust_avg
                best_pair = (s1, s2)
                best_metrics = {
                    'within_std': within_period_robust_avg,
                    'drift': total_drift,
    ...
    def detect_tilt_pattern(self, df):
        """Detect tilt sensor pattern using axis mapping (2+2 pattern)."""
        ...
        if self.structure_id and self.structure_id in self.TILT_AXIS_MAPPING:
            ...
            if len(x_sensors) >= 1 and len(y_sensors) >= 1:
                ...
                print(f"    Total: {len(x_sensors) + len(y_sensors)} sensors")
                return pattern
    ...
    def add_company_range_flags(self, features, df):
        ...
        if 'displacement_combined' in features.columns:
            disp_baseline = features['displacement_combined'].median()
            disp_deviation = (features['displacement_combined'] - disp_baseline).abs()
            displacement_violation = disp_deviation > 50
        ...
        if 'tilt_group_avg' in features.columns:
            tilt_baseline = features['tilt_group_avg'].median()
            tilt_deviation = (features['tilt_group_avg'] - tilt_baseline).abs()
            tilt_violation = tilt_deviation > 5
    ...
    def calculate_anomaly_score(self, features_df, scores):
        ...
        ensemble_score = (ensemble_votes / 4) * 4
    ...
    def classify_fault(self, features_df, row_idx, anomaly_score):
        """Classify fault type. Returns: (fault_type, confidence, reasoning)"""
    ...
    def train(self, df):
        ...
        self.trained_feature_cols = list(features_clean.columns)
        ...
        features_clean['ensemble_weighted_score'] = (
            self.ensemble_weights['isolation_forest'] * features_clean['if_flag'] +
            self.ensemble_weights['lof'] * features_clean['lof_flag'] +
            self.ensemble_weights['ocsvm'] * features_clean['svm_flag'] +
            self.ensemble_weights['threshold'] * features_clean['threshold_flag']
        )

        vote_cols = ['if_flag', 'lof_flag', 'svm_flag', 'threshold_flag']
        features_clean['ensemble_votes'] = features_clean[vote_cols].sum(axis=1)
        features_clean['final_anomaly'] = (features_clean['ensemble_weighted_score'] >= 0.5).astype(int)
...
def main(data_dir="."):
    """Run the Universal Model v3 on all structures. Test/demo function."""
    import os
