...
def merge_structure_with_all_sensors(structure_name, input_dir=".", output_dir="output"):
    """
    Merge all sheets (sensors) from all files in a structure.
    Creates one file per structure with sensor_id column.

    Args:
        structure_name: Structure identifier (e.g., 'STR122')
        input_dir: Directory containing structure folders (default: current directory)
        output_dir: Output directory for merged files (default: 'output')
    """
    base_path = os.path.join(input_dir, structure_name)
    ...
    write_log("="*80)
    write_log(f"MERGING STRUCTURE: {structure_name}")
    ...
    if not files:
        write_log(f"WARNING: No Excel files found - SKIPPING")
        with open(log_file, 'w') as f:
            f.write('\n'.join(log))
        return None
        ...
        try:
            ...
            for sheet_name in sheet_names:
                try:
                    ...
                    # Skip empty sheets
                    if len(df) == 0:
                        write_log(f"      Sheet '{sheet_name}': EMPTY - skipping")
                        continue
                    ...
                    if not date_col:
                        write_log(f"      Sheet '{sheet_name}': No timestamp column - skipping")
                        continue
                    ...
                    write_log(f"      OK Sheet '{sheet_name}': {len(df):,} rows, Type: {df['sensor_type'].iloc[0]}")

                except Exception as e:
                    write_log(f"      ERROR Sheet '{sheet_name}': {str(e)}")
                    continue
...
def merge_all_structures(input_dir=".", output_dir="output"):
    """
    Merge all structures with all sensors.

    Args:
        input_dir: Directory containing structure folders (e.g., STR122/, STR124/, ...)
        output_dir: Output directory for merged files
    """
    structures = sorted([d for d in os.listdir(input_dir)
                        if os.path.isdir(os.path.join(input_dir, d)) and d.startswith('STR')])
    ...
    for idx, structure in enumerate(structures, 1):
        ...
        result = merge_structure_with_all_sensors(structure, input_dir, output_dir)
        results.append(result)

        if result:
            print(f"\n{structure} completed: {result['sensors']} sensors, {result['final_rows']:,} rows")
        else:
            print(f"\nWARNING: {structure} had issues - check log")
    ...
    if failed:
        print(f"Failed/Skipped: {len(failed)} structures")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 3:
        ...
        merge_all_structures(input_dir, output_dir)
    elif len(sys.argv) == 2:
        input_dir = sys.argv[1]
        merge_all_structures(input_dir, "output")
    else:
        print("Usage: python merge_with_all_sensors.py <INPUT_DIR> [OUTPUT_DIR]")
        print("")
        print("Arguments:")
        print("  INPUT_DIR   Directory containing structure folders (STR122/, STR124/, ...)")
        print("  OUTPUT_DIR  Output directory for merged files (default: 'output')")
        print("")
        print("Example:")
        print("  python merge_with_all_sensors.py /path/to/raw_data /path/to/output")
        sys.exit(1)
