...
def analyze_complete_quality(structure_name, report_dir):
    """
    Complete data quality analysis with all checks.
    """
    ...
    for sensor_id in sensors:
        ...
        for col in data_columns:
            missing_count = sensor_df[col].isna().sum()
            missing_pct = (missing_count / len(sensor_df)) * 100

            if missing_count > 0:
                has_missing = True
                log_line(f"      {col}: {missing_count:,} missing ({missing_pct:.1f}%)")

                if missing_pct > 10:
                    issue = f"{col}: {missing_pct:.1f}% missing values"
                    sensor_info['issues'].append(issue)
                    log_line(f"         ⚠️  High missing rate")

        if not has_missing:
            log_line(f"      ✓ No missing values detected")
...
def generate_all_complete_reports():
    """Generate complete quality reports for all structures"""
    ...
    print("="*80)
    print("🚀 GENERATING COMPLETE DATA QUALITY REPORTS")
    print("="*80)
    ...
    for idx, structure in enumerate(structures, 1):
        ...
        report = analyze_complete_quality(structure, report_dir)
        if report:
    ...
    summary.append("   - Missing values detected in some sensors")
    ...
    summary.append("   - Handle missing values (interpolation for small gaps)")
    summary.append("   - Fill small data gaps using forward-fill or interpolation")
    ...
    summary.append("   - Account for missing data when training models")
    ...
    print("\n" + "="*80)
    print("✅ ALL COMPLETE REPORTS GENERATED!")
    print("="*80)
    print(f"\n📁 Location: {report_dir}")
